...
console.log("message sent"+data);
 ...
 socket.emit('welcome', { message: 'Welcome kumar!', id: socket.id });
    ...
    var query22="update [user]  set online='Y' where id='"+userinfo.id+"'";
			request.query(query22).then(function(re){
        			users.forEach(function(element, index, array){
	    	...
	    	for(var i=0;i<users.length;i++){
				if(users[i].id==uIdSocket){
					if(users[i].socketId==socket.id){
						spliceId=i;
						 var request = new sql.Request(connection);
			    	 request.query("update [user]  set online='N' where id='"+users[i].id+"'").then(function(re){
	...
	*/
	app.post('/get_userinfo', function(req, res){
		var	query="select id,name,p_photo,online from [user] where id='"+req.body.uid+"'";
		var request = new sql.Request(connection);
  request.query(query).then(function (result) {
	...
	*/
	...
	app.get('/logout', function(req, res){
       ...
       var request = new sql.Request(connection);
		request.query("update [user]  set online='N' where id='"+uid+"'").then(function(re){
